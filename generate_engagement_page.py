#!/usr/bin/env python3
"""
Generate engagement.html with category cards and slideshow data
"""

from pathlib import Path

def get_engagement_categories():
    """Get all engagement category folders and their images"""
    engagement_dir = Path('images/engagement')
    categories = {}
    
    for category_folder in sorted(engagement_dir.iterdir()):
...
def generate_engagement_html():
    """Generate the engagement.html file"""
    categories = get_engagement_categories()
    
    # Generate category cards HTML
    cards_html = []
    for cat_name, cat_data in categories.items():
        cards_html.append(f'''      <!-- {cat_name} Category -->
      <div class="category-card" data-category="{cat_name.lower().replace(' ', '-').replace('&', 'and')}">
    ...
    for cat_name, cat_data in categories.items():
        ...
        hidden_data_html.append(f'''    <div data-category="{cat_name.lower().replace(' ', '-').replace('&', 'and')}">
{images_html}
    </div>''')
...
<head>
  ...
  <title>Engagement - ThreePinFork</title>
...
<body>
  ...
  <main class="gallery-main">
    <div class="engagement-categories-grid">
...
{chr(10).join(hidden_data_html)}
  ...
  <script src="engagement-slideshow.js"></script>
</body>
</html>'''
    
    with open('engagement.html', 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"✅ Generated engagement.html with {len(categories)} categories")
